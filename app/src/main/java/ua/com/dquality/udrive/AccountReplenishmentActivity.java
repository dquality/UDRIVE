...
public class AccountReplenishmentActivity extends AuthenticateBaseActivity {
    ...
    private void updateAccountReplenishmentData(AccountReplenishmentModel model){
        ...
        amountValueText.setText(String.format(Const.AMOUNT_FORMAT, model.TotalAmount));
        ...
        sumValueText.setText(String.format(Const.AMOUNT_FORMAT, model.TotalAmount + model.CommissionAmount));
    }

    private View.OnClickListener onPerformAccountReplenishmentClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v){
            UDriveApplication.getHttpDataProvider().postAccountReplenishment(mViewModelData.getAccountReplenishmentModel().getValue());
        }
    };
}
